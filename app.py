...
# -----------------------------------------------------------------------------
# ניהול מצב גלובלי (Session State) לקטלוג הברזל
# -----------------------------------------------------------------------------
if 'iron_catalog' not in st.session_state:
    # הגדרת כמה פריטים ראשוניים כדוגמה לפעולה
...
# -----------------------------------------------------------------------------
# ניווט בין עמודים באפליקציה
# -----------------------------------------------------------------------------
page = st.sidebar.radio("ניווט במערכת:", ["💰 עמוד מחירון ומלאי ברזל", "📊 חישוב חיתוך ותמחור פרויקט"])

# אחוז רווח כללי לפרויקט
st.sidebar.markdown("---")
profit_margin = st.sidebar.slider("אחוז רווח מבוקש ללקוח (%):", min_value=0, max_value=200, value=50, step=5)

# =============================================================================
# עמוד 1: מחירון ומלאי ברזל (דינמי לחלוטין)
# =============================================================================
if page == "💰 עמוד מחירון ומלאי ברזל":
    st.title("📋 קטלוג ומחירון חומרי גלם")
    st.write("בעמוד זה אתה בונה ומעדכן את סוגי הברזל והמחירים שאתה קונה בהם מהספק. הנתונים כאן ישמשו את המחשבון באופן אוטומטי.")
    ...
    with col2:
        # התאמת שדה המידות בהתאם לסוג הברזל שנבחר כפי שביקשת
        ...
        else:  # מוטות מלאים מרובעים ועגולים - אין מידות רוחב/אורך אלא רק עובי
            dims = "-"
            st.caption("למוט מלא יש רק מידת עובי (בשדה הבא)")
            
    with col3:
        thickness = st.text_input("עובי הברזל (למשל 2 מ\"מ, 6 מ\"מ, 12 מ\"מ):", value="2 מ\"מ")
        
    with col4:
        price = st.number_input("מחיר קנייה למוט (₪):", min_value=0.0, value=100.0, step=5.0)
        
    # קביעת אורך המוט אוטומטית לפי חוקי השוק
    ...
    if st.button("💾 שמור ברזל זה למחירון", type="primary"):
        ...
        st.success("הברזל נוסף בהצלחה למחירון!")

    st.markdown("---")
    st.subheader("🗄️ מחירון הברזל הנוכחי שלך")
    
    if st.session_state.iron_catalog:
        df_catalog = pd.DataFrame(st.session_state.iron_catalog)
        # שינוי שמות העמודות שיוצגו יפה בעברית וממורכז
        df_catalog.columns = ["סוג ברזל", "מידות (בלי עובי)", "עובי ברזל", "מחיר קנייה למוט", "אורך מוט שוק (ס\"מ)"]
        st.dataframe(df_catalog, use_container_width=True, hide_index=False)
        
        if st.button("❌ מחק פריט אחרון מהמחירון"):
            if len(st.session_state.iron_catalog) > 0:
                st.session_state.iron_catalog.pop()
                st.rerun()
    else:
        st.info("המחירון ריק כרגע. הוסף ברזלים למעלה כדי להתחיל.")

# =============================================================================
# עמוד 2: מחשבון חיתוך ותמחור (כולל הזנה ידנית או AI אמיתי)
# =============================================================================
else:
    st.title("📊 מחשבון חיתוך ותמחור פרויקטים")
    
    if not st.session_state.iron_catalog:
        st.warning("⚠️ לא הגדרת אף ברזל במחירון! כנס קודם לעמוד 'מחירון ומלאי ברזל' כדי להוסיף חומרים.")
    else:
        # יצירת רשימת אפשרויות לבחירה מתוך המחירון שהמשתמש בנה
        material_options = [f"{item['type']} | {item['dimensions']} | {item['thickness']}" for item in st.session_state.iron_catalog]
        selected_material_idx = st.selectbox("בחר את סוג הברזל לפרויקט הנוכחי (מתוך המחירון שלך):", range(len(material_options)), format_func=lambda x: material_options[x])
        
        chosen_item = st.session_state.iron_catalog[selected_material_idx]
        max_len = chosen_item["length"]
        cost_per_bar = chosen_item["price"]
        
        st.info(f"💡 נבחר: **{chosen_item['type']}** במידה **{chosen_item['dimensions']}** (עובי {chosen_item['thickness']}). אורך מוט בשוק: {max_len} ס\"מ, מחיר קנייה: ₪{cost_per_bar}")
        ...
        with tab_manual:
            if 'calc_rows' not in st.session_state:
                st.session_state.calc_rows = [{'length': 90.0, 'qty': 5}, {'length': 10.0, 'qty': 5}]
                
            col_b1, col_b2 = st.columns(2)
            with col_b1:
                if st.button("➕ הוסף שורת מידה"):
            ...
            for i, row in enumerate(st.session_state.calc_rows):
                ...
                with r_col2:
                    q_val = st.number_input(f"כמות חיתוכים #{i+1}:", min_value=1, value=int(row['qty']), key=f"p_qty_{i}")
            ...
            if st.button("🚀 חשב תוכנית חיתוך ותמחור סופית", type="primary", key="btn_calc_manual"):
                ...
                if error_msg:
                    ...
                    # הצגת התוצאות הכספיות בכרטיסיות גדולות
                    c1, c2, c3 = st.columns(3)
                    with c1:
                        st.metric(label="כמות מוטות להזמנה", value=f"{total_bars} מוטות")
                    with c2:
                        st.metric(label="עלות קנייה כוללת (לפני מע\"מ)", value=f"₪ {total_cost:,.2f}")
                    with c3:
                        st.metric(label="מחיר מומלץ ללקוח (כולל רווח)", value=f"₪ {client_price:,.2f}")
                        
                    # טבלת תוכנית עבודה מפורטת
                    ...
                    for idx, bar in enumerate(bars_plan):
                        table_data.append({
                            ...
                            "סה\"כ מנוצל (ס\"מ)": f"{sum(bar)} ס\"מ",
                            "פחת/שארית שנשארת (ס\"מ)": f"{max_len - sum(bar)} ס\"מ"
                        })
                    st.dataframe(pd.DataFrame(table_data), use_container_width=True, hide_index=True)

        # --- טאב 2: ניתוח שרטוטים אמיתי (AI) ---
        with tab_drawing:
            st.subheader("📸 העלאת שרטוט לניתוח בינה מלאכותית")
            st.write("העלה צילום של שרטוט ידני מהשטח. המערכת תנתח אותו בזמן אמת ותחלץ את רשימת המידות היישר לתוך מנגנון החיתוך האופטימלי.")
            ...
            if uploaded_file is not None:
                ...
                if st.button("🔍 הפעל ניתוח AI אמיתי לשרטוט", type="primary"):
                    with st.spinner("ה-AI קורא ומנתח את השרטוט שלך..."):
                        try:
                            # חיבור אמיתי למודל הראייה של Gemini להוצאת הנתונים המדויקים מהתמונה שלך
                            # (ההנחיה מכריחה את המודל להחזיר רק נתוני JSON נקיים כדי שלא ימציא נתונים)
                            ...
                            Ignore architectural grids or grid lines, look only at the structural element measurements.
                            Return your result strictly in this valid JSON format, with no extra text or explanations:
                            {"detected_items": [{"length_cm": 150.0, "quantity": 2}]}
                            """
                            
                            response = model.generate_content([prompt, image])
                            
                            # ניקוי ה-Markdown של ה-JSON במידה והמודל החזיר אותו עטוף
                            ...
                            if "detected_items" in result_data and len(result_data["detected_items"]) > 0:
                                ...
                                # הרצת אלגוריתם החיתוך האופטימלי על המידות האמיתיות שיצאו מהשרטוט
                                bars_plan, _ = calculate_optimal_cutting(ai_cuts, max_len)
                                
                                total_bars = len(bars_plan)
                                total_cost = total_bars * cost_per_bar
                                client_price = total_cost * (1 + profit_margin / 100)
                                
                                # כרטיסיות סיכום לפענוח השרטוט
                                d1, d2, d3 = st.columns(3)
                                with d1:
                                    st.metric(label="מוטות נדרשים מהשרטוט", value=f"{total_bars} מוטות")
                                with d2:
                                    st.metric(label="עלות חומר גלם (₪)", value=f"₪ {total_cost:,.2f}")
                                with d3:
                                    st.metric(label="מחיר מומלץ ללקוח", value=f"₪ {client_price:,.2f}")
                                    
                                # הצגת טבלת חיתוך אופטימלית
                                st.subheader("📐 תוכנית חיתוך אופטימלית המבוססת על השרטוט:")
                                ai_table_data = []
                                for idx, bar in enumerate(bars_plan):
                                ...
                                st.error("ה-AI לא הצליח לזהות מידות ברורות בשרטוט. ודא שהמספרים כתובים בצורה ברורה ונסה שוב.")
                                
                        except Exception as e:
                            st.error(f"אירעה שגיאה בתקשורת עם מודל ה-AI: {str(e)}. ודא שהגדרת את ה-API Key שלך כראוי בסביבת העבודה.")
